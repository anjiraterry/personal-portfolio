        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 lg:grid-rows-3 gap-6 lg:h-[calc(100vh-140px)]">
              ...
              <div className="flex flex-col md:flex-row gap-8 h-full overflow-hidden">
                ...
                <div className="flex flex-col justify-center space-y-8 md:pr-4 flex-shrink-0">
                  <div>
                    ...
                    <p className="text-3xl font-display font-bold text-white/90">4+</p>
                  </div>
                  <div>
                    ...
                    <p className="text-3xl font-display font-bold text-white/90">4 Years</p>
                  </div>
                </div>
              </div>
            </BentoCard>
          ...
          <div className="md:col-span-2 lg:col-span-2 h-[450px] lg:h-full">
            <BentoCard noPadding className="h-full group relative overflow-hidden">
              ...
              {/* Layer 3: Content Slider (Top) - Synced with Image Slider */}
                ...
                <AnimatePresence initial={false} custom={direction}>
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        Explore Case Study 
                        <ArrowRight size={14} className="group-hover/case:translate-x-1 transition-transform" />
                      </Link>
                    </motion.div>
                  </motion.div>
                </AnimatePresence>
              ...
              <div className="absolute inset-0 flex items-center  justify-between px-1 pointer-events-none z-40">
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-black/30 backdrop-blur-md border border-white/10 flex items-center justify-center text-white pointer-events-auto hover:bg-black/60 transition-all opacity-0 group-hover:opacity-100 lg:-translate-x-2 group-hover:translate-x-0"
                >
                  <ChevronLeft size={15} />
                </button>
                
                <button
                  onClick={(e) => { e.preventDefault(); setShowProjectInfo(!showProjectInfo); }}
                  className={cn(
                    "w-8 h-8 rounded-full bg-white/5 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-[rgb(0,167,157,0.2)] transition-all pointer-events-auto z-50 opacity-0 group-hover:opacity-100",
                    showProjectInfo ? "absolute top-6 right-6 opacity-100" : "absolute bottom-6 right-10"
                  )}
                >
                  {showProjectInfo ? <ChevronDown size={15} /> : <ChevronUp size={15} />}
                </button>
          ...
          <div className="md:col-span-1 lg:col-span-2 h-[200px] md:h-[400px] lg:h-full">
            <BentoCard gradient glowOnHover={false} className="h-full flex flex-col justify-between p-6">
              <div className="overflow-y-auto no-scrollbar">
                ...
                <p className="text-white/40 text-xs leading-relaxed max-w-md line-clamp-3">{FOCUS_AREAS[0].description}</p>
              </div>
          ...
          <div className="md:col-span-1 lg:col-span-2 h-[250px] lg:h-full">
            <BentoCard className="h-full flex flex-col p-6">
              ...
              <div className="flex flex-wrap gap-1.5 overflow-y-auto custom-scrollbar pr-1">
                {activeTab === "stack" ? TECH_STACK.map((tech, i) => <TechBadge key={tech.name} name={tech.name} index={i} />) : EXPERTISE.map((item, i) => <TechBadge key={item} name={item} index={i} variant="teal" />)}
              </div>
            </BentoCard>
          </div>
